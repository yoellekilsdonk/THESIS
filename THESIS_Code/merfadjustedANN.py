...
class MERF(object):
    ...
    * bi is the random effect coefficients. They are different per cluster i but are assumed to be drawn from the same distribution ~N(0, Sigma_b) where Sigma_b is learned from the data.
    ...
    def __init__(
        ...
        self.currentrunname = currentrunname
    ...
    def predict(self, X: np.ndarray, Z: np.ndarray, clusters: pd.Series):
        ...
        In our case we make a split of train and test where an individual only occurs in one of the groups; we
        predict values of new individuals. Thus, we do not use the model to predict new values of existing individuals,
        although we could.
        ...
        y_hat = self.trained_fe_model.predict(X, use_multiprocessing=True, workers=multiprocessing.cpu_count(), verbose=0)
    ...
    def fit(
        ...
        for iteration in tqdm(range(0,self.max_iterations), ascii= False, desc="EM-algorithm MeANN"):
            ...
            self.fe_model.fit(X, y_star, use_multiprocessing=True, workers=multiprocessing.cpu_count(), verbose=0)
            del self.fe_model
            self.fe_model = tf.keras.models.load_model(self.currentrunname)
            f_pred = self.fe_model.predict(X, use_multiprocessing=True, workers=multiprocessing.cpu_count(), verbose=0) # UPDATE F_HAT
            f_hat = f_pred.reshape([-1,])
